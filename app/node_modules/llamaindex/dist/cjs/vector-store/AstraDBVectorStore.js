...
class AstraDBVectorStore extends _types.BaseVectorStore {
    storesText = true;
    flatMetadata = true;
    idKey;
    contentKey;
   ...
   */ async query(query, options) {
    ...
    toAstraFilter(filters) {
        ...
        const condition = filters.condition ?? _types.FilterCondition.AND;
        const listFilter = filters.filters.map((f)=>this.buildFilterItem(f));
        if (condition === _types.FilterCondition.OR) return {
            $or: listFilter
        };
        if (condition === _types.FilterCondition.AND) return {
            $and: listFilter
        };
    ...
    buildFilterItem(filter) {
        ...
        switch(operator){
            case _types.FilterOperator.EQ:
                return {
                    [key]: value
                };
            case _types.FilterOperator.NE:
                return {
                    [key]: {
                        $ne: value
                    }
                };
            case _types.FilterOperator.GT:
                return {
                    [key]: {
                        $gt: value
                    }
                };
            case _types.FilterOperator.LT:
                return {
                    [key]: {
                        $lt: value
                    }
                };
            case _types.FilterOperator.GTE:
                return {
                    [key]: {
                        $gte: value
                    }
                };
            case _types.FilterOperator.LTE:
                return {
                    [key]: {
                        $lte: value
                    }
                };
            case _types.FilterOperator.IN:
                return {
                    [key]: {
                        $in: (0, _utils.parseArrayValue)(value)
                    }
                };
            case _types.FilterOperator.NIN:
                return {
                    [key]: {
                        $nin: (0, _utils.parseArrayValue)(value)
                    }
                };
            case _types.FilterOperator.IS_EMPTY:
                return {
                    [key]: {
                        $size: 0
                    }
