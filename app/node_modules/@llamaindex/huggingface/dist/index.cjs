 ...
 */ class HuggingFaceEmbedding extends embeddings.BaseEmbedding {
    constructor(init){
        super(), this.modelType = HuggingFaceEmbeddingModelType.XENOVA_ALL_MINILM_L6_V2, this.quantized = true;
        Object.assign(this, init);
    }
    async getExtractor() {
        if (!this.extractor) {
            const { pipeline } = await env.loadTransformers((transformer)=>{
...
class HuggingFaceLLM extends llms.BaseLLM {
    ...
    async getModel() {
        const { AutoModelForCausalLM } = await env.loadTransformers((transformer)=>{
            global.Settings.callbackManager.dispatchEvent("load-transformers", {
                transformer
            }, true);
        });
