 ...
 */ class AssemblyAIReader {
   ...
   */ constructor(assemblyAIOptions){
        ...
        if (!options.apiKey) {
            options.apiKey = env.getEnv("ASSEMBLYAI_API_KEY");
        }
        if (!options.apiKey) {
        ...
        this.clientPromise = import('assemblyai').then(({ AssemblyAI })=>new AssemblyAI({
                ...defaultOptions,
                ...options
            }));
    }
 ...
 */ class AudioTranscriptParagraphsReader extends AssemblyAIReader {
   ...
   */ async loadData(params) {
        ...
        const client = await this.clientPromise;
        const paragraphsResponse = await client.transcripts.paragraphs(transcriptId);
        return paragraphsResponse.paragraphs.map((p)=>new schema.Document({
                text: p.text
            }));
    }
}
 ...
 */ class AudioTranscriptSentencesReader extends AssemblyAIReader {
   ...
   */ async loadData(params) {
        ...
        return sentencesResponse.sentences.map((p)=>new schema.Document({
                text: p.text
            }));
    }
}
 ...
 */ class AudioSubtitlesReader extends AssemblyAIReader {
   ...
   */ async loadData(params, subtitleFormat = "srt") {
        const transcriptId = await this.getTranscriptId(params);
        const client = await this.clientPromise;
        const subtitles = await client.transcripts.subtitles(transcriptId, subtitleFormat);
        return [
            new schema.Document({
                text: subtitles
            })
